...
import { confirmImportedResultMatchAction, resetImportedResultsAction, setImportedResultDisqualificationAction, verifyImportedResultsAction } from "@/app/admin/tournament-manager/import/workflow-actions";
...
import type { WeighfishReconciliation } from "@/lib/weighfish-reconciliation";
...
type Props = { tournamentId: string; tournamentSlug: string; rows: ImportedRow[]; verified: boolean; published: boolean; reconciliation?: WeighfishReconciliation; roster?: { id: string; boatNumber: number | null; displayName: string }[] };

export default function ImportedResultsReview({ tournamentId, rows, verified, published, reconciliation: reconciliationProp, roster = [] }: Props) {
  const reconciliation = reconciliationProp ?? { activeEntryCount: 0, importedResultCount: rows.length, autoMatchedCount: 0, manuallyMatchedCount: 0, missingResults: [], unmatchedImports: [], unresolvedRows: [], duplicateRows: [], rows: [], ready: verified };
  ...
  function confirmMatch(resultEntryId: string, registrationId: string) { if (!registrationId) return; setMessage(""); startTransition(async () => { try { await confirmImportedResultMatchAction(tournamentId, resultEntryId, registrationId); setMessage("Roster match saved."); router.refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : "Roster match could not be saved."); } }); }
  ...
  return <AdminPanel className="p-5 sm:p-7">
    ...
    <section className="mt-5 border border-white/10 bg-black/30 p-4" aria-label="WeighFish reconciliation summary">
      <h3 className="text-sm font-black uppercase text-white">Roster Reconciliation</h3>
      <div className="mt-3 grid grid-cols-2 gap-3 text-xs sm:grid-cols-4 lg:grid-cols-7">
        <Summary label="AITT Active Entries" value={reconciliation.activeEntryCount} />
        <Summary label="WeighFish Results" value={reconciliation.importedResultCount} />
        <Summary label="Auto Matched" value={reconciliation.autoMatchedCount} />
        <Summary label="Manually Matched" value={reconciliation.manuallyMatchedCount} />
        <Summary label="Missing Results" value={reconciliation.missingResults.length} alert={reconciliation.missingResults.length > 0} />
        <Summary label="Unmatched Imports" value={reconciliation.unmatchedImports.length} alert={reconciliation.unmatchedImports.length > 0} />
        <Summary label="Unresolved" value={reconciliation.unresolvedRows.length} alert={reconciliation.unresolvedRows.length > 0} />
      </div>
      {reconciliation.rows.filter((row) => row.fuzzy && row.outcome === "auto").map((row) => <p key={row.resultId} className="mt-3 text-xs text-emerald-300"><strong>AUTO MATCHED</strong> — WeighFish: {row.importedName} · AITT: {row.candidates[0]?.displayName}</p>)}
      {reconciliation.unresolvedRows.map((row) => <div key={row.resultId} className="mt-4 border border-amber-400/25 bg-amber-400/5 p-4"><p className="text-xs font-black uppercase text-amber-300">{row.outcome === "unmatched" ? "Unmatched WeighFish Import" : "Manual Review Required"}</p><p className="mt-2 text-sm text-white"><span className="text-neutral-400">WEIGHFISH IMPORTED:</span> {row.importedName}</p><p className="mt-2 text-xs font-black uppercase text-neutral-400">Likely AITT Matches</p><select aria-label={`AITT roster match for ${row.importedName}`} defaultValue="" className="mt-2 min-h-11 w-full border border-white/15 bg-black px-3 text-sm text-white" onChange={(event) => confirmMatch(row.resultId, event.target.value)} disabled={pending}><option value="">Choose the correct boat / registration</option>{roster.map((entry) => <option key={entry.id} value={entry.id}>Boat #{entry.boatNumber ?? "—"} — {entry.displayName}</option>)}</select><p className="mt-2 text-xs text-neutral-400">{row.reason}</p></div>)}
      {reconciliation.missingResults.map((entry) => <div key={entry.id} className="mt-4 border border-red-500/30 bg-red-500/5 p-4"><p className="text-xs font-black uppercase text-red-300">Missing WeighFish Result</p><p className="mt-2 text-sm font-semibold text-white">Boat #{entry.boatNumber ?? "—"} — {entry.displayName}</p><p className="mt-2 text-xs text-neutral-400">Correct WeighFish, export a complete CSV, then reset and re-import. AITT will not invent a scoring row.</p></div>)}
    </section>
    ...
    <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">{verified ? <><p><AdminStatusBadge tone="positive">✓ Results Verified</AdminStatusBadge></p><p className="text-sm text-neutral-400">{rows.length} Teams Imported</p></> : <button type="button" onClick={verify} disabled={pending || !reconciliation.ready} className={adminButtonStyles("primary", "min-h-11 px-5")}>{pending ? "Working…" : "Verify Imported Results"}</button>}<button type="button" aria-expanded={resultsExpanded} onClick={toggleResults} className={adminButtonStyles("secondary")}>{resultsExpanded ? "Collapse" : verified ? "Edit" : "Expand"}</button></div>
    {message ? <p role="status" className="mt-4 text-sm text-neutral-300">{message}</p> : null}
  </AdminPanel>;
}

function Summary({ label, value, alert = false }: { label: string; value: number; alert?: boolean }) { return <div className="border border-white/10 bg-black/40 p-3"><p className="text-[10px] font-black uppercase tracking-wide text-neutral-500">{label}</p><p className={`mt-1 text-lg font-black ${alert ? "text-red-300" : "text-white"}`}>{value}</p></div>; }
