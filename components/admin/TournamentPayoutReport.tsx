  ...
  return <div className="mt-6 border-t border-white/10 pt-6">
    ...
    {expanded ? <div id="tournament-payout-summary-details" className="mt-4 space-y-5">
      {categories.map((category) => {
        ...
        return <section key={category}>
          ...
          <div className="divide-y divide-white/10">{checks.map((check, index) => <div key={check.id} className="grid gap-1 py-2.5 sm:grid-cols-[8rem_1fr_auto] sm:items-center sm:gap-4">
    ...
    <section className="mt-6 border-t border-white/15 pt-5">
      ...
      {checksExpanded ? <div id="saved-check-summary-details" className="mt-4 divide-y divide-white/10 border-y border-white/10">
        {[...groupedRecipients.values()].sort((left, right) => Math.min(...left.map((check) => check.finishingPlace)) - Math.min(...right.map((check) => check.finishingPlace))).map((checks) => <article key={checks[0].entryName} className="py-4">
