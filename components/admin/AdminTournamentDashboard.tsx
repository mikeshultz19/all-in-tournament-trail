...
export default function AdminTournamentDashboard({ tournaments, initialTournamentId, comparisonDate, showTournamentTools = false, closeouts = {}, insuranceResults = {}, importEvidence = {}, initialExpandedStage, supplementalEvidence = {}, registrationSummaries = {}, importedRows = {}, resultsRecords = {}, manualReviewRows = {}, publishReviewRegistrations = {}, collectionSummaries = {}, aoyStandingsBySeason = {}, championshipQualificationsBySeason = {} }: AdminTournamentDashboardProps) {
...
function PayoutStage({ tournament, importedRows, closeout, collectionSummary, insuranceResult, locked, publicationExists }: { tournament: Tournament; importedRows: ImportedRow[]; closeout?: OnSiteCloseoutRecord; collectionSummary: TournamentCollectionSummary | null; insuranceResult?: TournamentInsurancePotResultRecord | null; locked: boolean; publicationExists: boolean }) {
  const identifier = encodeURIComponent(tournament.slug || tournament.id);
  ...
  return <div className="max-w-6xl space-y-8">
    <PayoutFundsComparison summary={collectionSummary} closeout={closeout} />
...
function PayoutFundsComparison({ summary, closeout }: { summary: TournamentCollectionSummary | null; closeout?: OnSiteCloseoutRecord }) {
  if (!summary) return null;
  const checks = closeout?.checks.reduce((total, check) => total + check.amountCents, 0) ?? null;
  return <section className="rounded-md border border-white/10 bg-[#111] p-4 sm:p-5" aria-labelledby="payout-funds-comparison-heading"><p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#D4A017]">Read-Only Comparison</p><h3 id="payout-funds-comparison-heading" className="mt-1 text-base font-black uppercase text-white">Tournament Funds vs. Payout Checks</h3><dl className="mt-4 grid gap-3 text-sm sm:grid-cols-3"><div><dt className="text-[10px] font-black uppercase tracking-[0.12em] text-neutral-500">Total Tournament Payout Funds</dt><dd className="mt-1 font-black tabular-nums text-white">{money(summary.totalTournamentPayoutFundsCents)}</dd></div><div><dt className="text-[10px] font-black uppercase tracking-[0.12em] text-neutral-500">Calculated Payout Checks</dt><dd className="mt-1 font-black tabular-nums text-white">{checks === null ? "Not saved" : money(checks)}</dd></div><div><dt className="text-[10px] font-black uppercase tracking-[0.12em] text-neutral-500">Unreconciled Difference</dt><dd className="mt-1 font-black tabular-nums text-[#D4A017]">{checks === null ? "Not available" : money(summary.totalTournamentPayoutFundsCents - checks)}</dd></div></dl><p className="mt-3 text-xs leading-5 text-neutral-500">Membership revenue and online service fees are excluded. A difference may reflect retained funds or incomplete payout calculations.</p></section>;
}
