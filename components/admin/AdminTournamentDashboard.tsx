...
function PrepareStage({ tournament, identifier, summary = { total: 0, paid: 0, needReview: 0 }, undoBlockers }: { tournament: Tournament; identifier: string; summary?: TournamentRegistrationRosterSummary; undoBlockers: string[] }) {
  return <div className="max-w-4xl space-y-5"><section><h3 className="font-bold text-white">Online Registration Roster</h3><dl className="mt-3 grid grid-cols-3 gap-3 text-sm"><CompactValue label="Teams" value={summary.total} /><CompactValue label="Paid" value={summary.paid} /><CompactValue label="Need Review" value={summary.needReview} /></dl><div className="mt-5 flex flex-wrap gap-3"><Link href={`/admin/tournament-manager/prepare?tournament=${identifier}`} className={adminButtonStyles("primary", "min-h-11")}>View Registration Roster</Link><Link href={`/admin/tournament-manager/prepare?tournament=${identifier}`} className={adminButtonStyles("secondary", "min-h-11")}>Print Check-In List</Link></div></section><PrepareMembershipReminder tournamentId={tournament.id} tournamentName={tournament.name} tournamentIdentifier={identifier} needReviewCount={summary.needReview} hasExistingImport={Boolean(tournament.weighfish_imported || tournament.weighfish_imported_at)} initialRegistrationReviewComplete={Boolean(tournament.prepare_registration_review_complete)} initialPaperMembershipsConfirmed={Boolean(tournament.paper_membership_reminder_checked)} undoBlockers={undoBlockers} returnHref={`/admin/tournament-manager?tournament=${identifier}&step=1`} /></div>;
}
