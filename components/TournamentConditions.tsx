...
function weatherDayLabel(day: TournamentWeatherDay) {
  ...
  return `${formatForecastWeekday(day.date)}, ${day.conditionText}${
...
export default function TournamentConditions({
  ...
  return (
    ...
    >
          ...
          <p className="inline-flex items-center gap-2 border border-white/15 px-2 py-1 text-[10px] font-black uppercase tracking-[0.1em] text-white">
            <span
              ...
              className={`h-1.5 w-1.5 rounded-full ${
                STATUS_INDICATOR_STYLES[tournament.tournamentStatus]
              }`}
            />
      ...
      <div
        data-conditions-row
        className="grid min-w-0 gap-3 pt-3 md:grid-cols-[minmax(145px,0.28fr)_minmax(0,1fr)] md:divide-x md:divide-white/10 min-[1360px]:grid-cols-[minmax(170px,0.34fr)_minmax(0,1fr)]"
      >
        ...
        <div className="min-w-0 md:pl-3">
          <h3 className="text-[11px] font-black uppercase tracking-[0.12em] text-neutral-500">
            <span className="min-[1360px]:hidden">Next 3 Days</span>
            <span className="hidden min-[1360px]:inline">Next 5 Days</span>
          </h3>

          {forecast ? (
            <>
              <div className="mt-2 max-w-full">
                <ol className="grid grid-cols-3 gap-1.5 min-[1360px]:grid-cols-5">
                  {forecast.days.map((day, index) => (
                    <li
                      ...
                      className={`min-w-0 border border-white/10 bg-black/30 px-1 py-2 text-center ${
                        index >= 3 ? "hidden min-[1360px]:block" : ""
                      }`}
                    >
                      <time
                        dateTime={day.date}
