...
export function buildRegistrationConfirmationEmail(view: RegistrationConfirmationEmailView) {
  ...
  return {
    ...
    html: `<div style="background:#f4f4f4;padding:24px;font-family:Arial,Helvetica,sans-serif;color:#111"><div style="max-width:640px;margin:0 auto;background:#fff;padding:28px"><h1 style="margin:0;color:#16803a;font-size:26px">You’re Registered</h1><div style="margin-top:22px;padding:12px;background:#111;color:#fff"><div style="font-size:11px;font-weight:700;color:#d4a017;text-transform:uppercase">REGISTRATION NUMBER</div><div style="margin-top:5px;font-size:22px;font-weight:700">${view.boatNumber ? `#${view.boatNumber}` : "TBA"}</div></div><p style="font-size:14px;line-height:1.6">${opening}</p><h2 style="margin:28px 0 8px;font-size:18px">Tournament Information</h2><div style="margin:0 0 14px;border:1px solid #d4a017;background:#fff8e6;padding:12px 14px"><div style="font-size:12px;font-weight:700;color:#9b7300;text-transform:uppercase;letter-spacing:0.04em">Fish Length Requirements</div><ul style="margin:10px 0 0;padding:0 0 0 18px;font-size:14px;line-height:1.6;color:#111"><li>Largemouth Bass: 14-inch minimum</li><li>Smallmouth Bass: 14-inch minimum</li><li>Spotted Bass: No minimum length</li></ul></div><table role="presentation" style="width:100%;border-collapse:collapse">${row("Tournament / Lake", tournamentLake)}${row("Date", date)}${row("Ramp / Launch Location", view.ramp)}${row("Launch Type", view.launchType)}${row("Morning Registration / Check-In", view.morningRegistration)}${safeLight}${row("Scales Close / Weigh-In", view.scalesClose)}${row("Registered Anglers", view.anglers.join(" / "))}${row("Memberships / Options / Pots", view.selectedOptions.join(", "))}${paymentMethod}${row("Amount Paid", formatCurrencyFromCents(view.totalCents))}</table><p style="margin-top:20px;font-size:12px;font-style:italic;color:#666">All tournament times are subject to change by the Tournament Director.</p><h2 style="margin:26px 0 8px;font-size:18px">Tournament Status</h2><p style="font-size:14px;line-height:1.6;color:#333">Check the AITT homepage before the tournament for weather-related postponements, cancellations, or schedule changes. Updates will also be posted on AITT social media.</p><p><a href="${AITT_HOMEPAGE_URL}" target="_blank" rel="noopener noreferrer" style="color:#9b7300;font-weight:700">Check AITT Homepage →</a></p></div></div>`,
  };
}
