...
export async function POST(request: Request) {
  let input: { visitorId?: string; sessionId?: string; path?: string; pageName?: string; referrer?: string; utmSource?: string | null };
  ...
  const utmSource = input.utmSource?.trim().slice(0, 255) || null;
  const { error: sessionError } = await supabase.from("website_analytics_sessions").upsert({
    ...
    referrer_domain: referrerDomain, utm_source: utmSource, last_seen_at: new Date().toISOString(),
