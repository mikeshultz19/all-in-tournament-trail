...
export function validateStagingEnvironment(environment) {
  ...
  for (const name of PUBLIC_STAGING_VARIABLES.concat(ENCRYPTED_STAGING_SECRETS)) required(environment, name);
  return { appUrl };
}
...
function main() {
  ...
  const { appUrl } = validateStagingEnvironment(process.env);
