...
describe("registration membership review preservation", () => {
  ...
  it("retains duplicate-purchase review without obsolete side-pot eligibility markers", () => {
    const duplicate = [{ participantPosition: 1 as const, reason: "Possible Duplicate Membership Purchase: already active" }];
    expect(preserveUnresolvedMembershipReviews([angler("joining")], duplicate, new Set([1]))).toEqual(duplicate);
    expect(preserveUnresolvedMembershipReviews([angler("current")], [], new Set([1]))[0].reason)
      .not.toContain("Member-only");
  });
});
