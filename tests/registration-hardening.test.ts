...
const walkUpFollowUpMigration = readFileSync("supabase/migrations/202609250002_allow_selected_current_member_without_email.sql", "utf8");
...
describe("registration hardening", () => {
  ...
  it("allows a selected current member without email while still requiring email for Joining", () => {
    ...
    expect(walkUpFollowUpMigration).toContain("create or replace function public.admin_create_sequential_walkup_registration");
    expect(walkUpFollowUpMigration).toContain("AITT_WALKUP_SELECTED_MEMBER_REQUIRED");
