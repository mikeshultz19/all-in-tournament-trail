...
| Membership creation and eligibility | Admin Members routes and review actions; `lib/memberships.ts` | `members`, `memberships`, individual snapshots | Existing member and Not Member add $0; Confirm Membership Purchase records exactly the collected $40; no shortfall/receivable | membership-review tests; live decision rehearsal remains required |
...
| AOY standings | `lib/aoy-engine-core.ts`, AOY routes | Competitive records and approved results | Uses ordinary result eligibility and approved membership rules; no No Show application status | `tests/aoy-engine.test.ts`; staging result rehearsal required |
| Championship eligibility | `lib/championship-qualification-core.ts` | Season results and qualification records | Uses approved appearances/results; no No Show application branch | `tests/championship-qualification.test.ts`; staging rehearsal required |
