# AITT Knowledge Base

**First Edition — Inaugural Season**

## Purpose

This knowledge base supports the primary human-readable authority,
[AITT Lifecycle and Operations](../AITT_LIFECYCLE_OPERATIONS.md). It explains how to prepare a tournament, manage members and entries,
import and validate WeighFish results, prepare payouts, close out an event,
publish results, and update season standings without requiring software
knowledge.

It is written for the founder/product owner, tournament-operating partner, the
founder's wife when assisting the business, and one additional tournament
assistant.

**Internal principle:** “Simple enough to use. Professional enough to trust.”

**Operational principle:** “Technology should quietly make the tournament
better.”

**Tournament-day priority:** After weigh-in, import and verify the WeighFish
CSV, resolve required identity reviews, calculate payouts, prepare checks, and
complete financial closeout first. Anglers are waiting to be paid. Website
publication, photos, AOY, and Championship updates come afterward.

## Recommended reading order

1. [About AITT](01-ABOUT-AITT.md) — the trail's operating principles.
2. [Running a Tournament](02-RUNNING-A-TOURNAMENT.md) — the complete event
   sequence and stop conditions.
3. [Memberships and Registration](03-MEMBERSHIPS-AND-REGISTRATION.md) — member
   identity, eligibility, entries, and the official field.
4. [Results, AOY, and Championship](04-RESULTS-AOY-AND-CHAMPIONSHIP.md) — what
   publication means and how public records must agree.
5. [Admin Center Guide](05-ADMIN-CENTER-GUIDE.md) — what each current Admin
   screen does.
6. [Troubleshooting](06-TROUBLESHOOTING.md) — safe responses to common
   operational problems.
7. [Glossary](APPENDIX-GLOSSARY.md) — consistent AITT terminology.
8. [Launch Test Plan](AITT-LAUNCH-TEST-PLAN.md) — the required realistic
   tournament simulations.

## How to use this guide

Read the applicable procedure before changing live tournament data. Verify the
result in both Admin and the public website. Stop when a listed stop condition
occurs; do not work around missing or inconsistent data by inventing values.

The protected Admin Center, verified Square registration boundary, WeighFish
import/review, identity reconciliation, payout closeout, Official Results, AOY,
and Championship engines are implemented. Production payment enablement is a
separate controlled rollout; only a verified completed payment creates an entry.

Technical implementation and repository maintenance belong in the separate
developer documentation path listed in
[Documentation Index](../DOCUMENTATION-INDEX.md).
