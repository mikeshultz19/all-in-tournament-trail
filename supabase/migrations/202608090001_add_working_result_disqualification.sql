-- Explicit Admin-only disqualification workflow for verified Working Results.
-- DQ rows remain in Working Results/audit history but never enter Official Results.

alter table public.working_result_audit
  drop constraint if exists working_result_audit_action_check;
alter table public.working_result_audit
  add constraint working_result_audit_action_check check (
    action in (
      'import', 'replace_import', 'correction', 'identity_resolution',
      'mark_disqualified', 'remove_disqualification'
    )
  );

create or replace function public.enforce_disqualified_working_result()
returns trigger
language plpgsql
set search_path = public, pg_temp
as $$
begin
  if new.participation_status = 'disqualified' then
    new.aoy_eligible := false;
    new.base_payout := 0;
    new.bronze_payout := 0;
    new.silver_payout := 0;
    new.gold_payout := 0;
    new.big_bass_payout := 0;
...
drop trigger if exists working_result_enforce_disqualification
  on public.tournament_result_entries;
create trigger working_result_enforce_disqualification
before insert or update on public.tournament_result_entries
for each row execute function public.enforce_disqualified_working_result();

create or replace function public.omit_disqualified_official_result()
returns trigger
language plpgsql
set search_path = public, pg_temp
as $$
begin
  if exists (
    select 1 from public.tournament_result_entries working
    where working.id = new.working_result_entry_id
      and working.participation_status = 'disqualified'
  ) then
    return null;
  end if;
  -- Re-rank the remaining field without exposing a gap where a DQ finished.
  if new.place is not null then
    select count(*) + 1 into new.place
    from public.tournament_result_entries working
    where working.tournament_id = new.tournament_id
      and working.participation_status <> 'disqualified'
      and working.place < new.place;
  end if;
  return new;
end;
$$;

drop trigger if exists official_result_omit_disqualification
  on public.official_result_entries;
create trigger official_result_omit_disqualification
before insert on public.official_result_entries
for each row execute function public.omit_disqualified_official_result();
...
begin
  ...
  if p_disqualified and v_previous.participation_status <> 'disqualified' then
    ...
    update public.tournament_result_entries set
      participation_status = coalesce(v_restore ->> 'participation_status', 'participated'),
      aoy_eligible = coalesce((v_restore ->> 'aoy_eligible')::boolean, true),
      aoy_eligibility_snapshot = v_restore -> 'aoy_eligibility_snapshot',
...
comment on function public.set_working_result_disqualification(uuid, uuid, boolean, uuid) is
  'Explicitly marks or removes DQ on a verified Working Result before financial closeout/publication; never infers DQ from weight.';
